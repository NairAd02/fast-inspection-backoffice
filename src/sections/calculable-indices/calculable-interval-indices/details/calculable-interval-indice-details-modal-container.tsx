...
import CalculableIntervalDetailsContainer from "./calculable-interval-indice-details-container";

export default function CalculableIntervalIndiceDetailsModalContainer() {
  const { getInfoModal } = useContext(ModalContext);
  const infoModal = getInfoModal(
    modalTypes.detailsCalculableIntervalIndiceModal.name
  );
    ...
    <div className="flex flex-1 flex-col h-full w-full">
      {!loading ? (
        calculableIntervalIndice && !error ? (
          <CalculableIntervalDetailsContainer
            calculableIntervalIndiceDetails={calculableIntervalIndice}
          />
        ) : (
